...
def cv_kNN(X, y):
    ...
    for k in range(n_k):
        ...
        for train_ind, val_ind in cv_kf.split(X, y):
            X_train = X[train_ind]
            X_val = X[val_ind]
            y_train = y[train_ind]
            y_val = y[val_ind]
...
def cal_RRMSE(M, N, S_0, K, H, mu, sigma, r, tau, T, alpha, L0, seed):
    ...
    loss = broadie_knn(M, N, S_0, K, H, mu, sigma, r, tau, T, rng.integers(0, 2147483647), rng.integers(0, 2147483647))
...
for j in range(n_trials):
    ...
    res = Parallel(n_jobs=n_cores, verbose=0)(delayed(cal_RRMSE)(M, N,
    ...
    print("KNN Estimation Done for:", M, "x", N)
    print(RRMSE)

print(result_table)
np.save(save_path + "result_154.npy", result_table)
