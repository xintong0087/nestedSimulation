...
def gbr_Euro_Call(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T):
    ...
    learningRate_opt = 0.1
    nEstimators_opt = 100

    rf = GradientBoostingRegressor(n_estimators=n_opt, max_depth=d_opt, max_features=f_opt).fit(X_train, y_train)
...
def cv_rf(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T):
    ...
    param_distributions = {
        "learning_rate": Real(1e-5, 1e-1, "log_uniform"),
        "n_estimators": ["auto", "sqrt"],
        "max_depth": [int(x) for x in np.linspace(5, 101, 21)]
    }
