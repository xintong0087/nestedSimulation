...
def SNS_Euro_Call_BootStrap(Gamma, n_front_vec, n_back_vec, d, S_0, K, mu, sigma, r, tau, T,
    ...
    alpha_mat = np.zeros([outer_shape, 3+2*len(level_list)])

    counter = 0
    for n_back in n_back_vec:

        res = np.zeros([I, 3+2*len(level_list)])
        ...
        for i in range(I):
            ...
            for j in range(len(level_list)):
                res[i, 3 + j] = loss_bs[int(np.ceil(level_list[j] * n_front_0)) - 1]
                res[i, 4 + j] = np.nanmean(loss_bs[loss_bs >= res[i, 3 + j]])

        alpha_mat[counter, :] = np.mean(res, axis=0)
        counter = counter + 1

    inner_shape = n_back_vec.shape[0]
    s_mat = np.zeros([inner_shape, 3+2*len(level_list)])
    ...
    for n_front in n_front_vec:
        ...
        for i in range(I):
            ...
            loss_bs.sort()
            for j in range(len(level_list)):
                res[i, 3 + j] = loss_bs[int(np.ceil(level_list[j] * n_front)) - 1]
                res[i, 4 + j] = np.nanmean(loss_bs[loss_bs >= res[i, 3 + j]])

        s_mat[counter, :] = np.var(res, axis=0)
        counter = counter + 1

    n_front_opt = np.zeros(3+2*len(level_list))
    n_back_opt = np.zeros(3+2*len(level_list))

    for i in range(3+2*len(level_list)):
        ...
        try:
            ...
            n_front_opt[i] = int(Gamma ** (2 / 3))
            n_back_opt[i] = int(Gamma ** (1 / 3))
...
def cal_RRMSE(Gamma, n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, level_list, L0):
    ...
    CVaR = {}
    for j in range(len(level_list)):
        loss = SNS_Euro_Call(n_front_opt[3+j], n_back_opt[3+j], d, S_0, K, mu, sigma, r, tau, T)
        loss.sort()
        VaR[level_list[j]] = loss[int(np.ceil(level_list[j] * n_front_opt[3+j])) - 1]
        CVaR[level_list[j]] = np.mean(loss[loss >= VaR[level_list[j]]])

    res = [indicator, hockey, quadratic]

    for level in level_list:
        res.append(VaR[level])
        res.append(CVaR[level])
...
#     res = Parallel(n_jobs=8, verbose=10)(delayed(cal_RRMSE)(Gamma_11, n_front_vec[_j], n_back_vec[_j],
#                                                             d_11, S_0_11, K_11,
