...
def lr_Geo_Asian(n_front, n, d, S_0, K, mu, sigma, r, tau, T, h):
    ...
    # A = np.linalg.cholesky(cor_mat).T
...
def cal_RRMSE(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, alpha, L0):
    ...
    loss = lr_Geo_Asian(n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h)
...
n_front_list = [80000, 90000, 100000]
...
for _j in range(n_trials):
    ...
    res = Parallel(n_jobs=50, verbose=100)(delayed(cal_RRMSE)(n_front_12, n_12, d_12, S_0_12, K_12,
...
print(result_table)
np.save("data/result_125.npy", result_table)
