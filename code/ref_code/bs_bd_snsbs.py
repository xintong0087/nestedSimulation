...
def cal_RRMSE(Gamma, n_front, n_back, d, S_0, K, mu, sigma, r, tau, T, h, H, alpha, L0):
    ...
    loss = SNS_Barrier_D(n_front_opt[0], n_back_opt[0], d, S_0, K, mu, sigma, r, tau, T, h, H)
    indicator = np.nanmean((loss > L0))

    loss = SNS_Barrier_D(n_front_opt[1], n_back_opt[1], d, S_0, K, mu, sigma, r, tau, T, h, H)
    hockey = np.nanmean(np.maximum(loss - L0, 0))

    loss = SNS_Barrier_D(n_front_opt[2], n_back_opt[2], d, S_0, K, mu, sigma, r, tau, T, h, H)
    quadratic = np.nanmean((loss - L0) ** 2)

    loss = SNS_Barrier_D(n_front_opt[3], n_back_opt[3], d, S_0, K, mu, sigma, r, tau, T, h, H)
    loss.sort()
    VaR = loss[int(np.ceil((1 - alpha) * n_front_opt[3]))]

    loss = SNS_Barrier_D(n_front_opt[4], n_back_opt[4], d, S_0, K, mu, sigma, r, tau, T, h, H)
    loss.sort()
...
print(result_table)
np.save("data/result_142.npy", result_table)
