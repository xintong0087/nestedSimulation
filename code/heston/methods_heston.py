...
def Heston_front_1d(n_front, S_0, drift, V_0, rho, kappa, theta, xi, tau, step_size=1 / 253):

    n_step = int(tau // step_size) + 1
    S_front = np.zeros([n_front, n_step + 1])
    S_front[:, 0] = np.array(S_0)

    V_front = np.zeros_like(S_front)
    V_front[:, 0] = np.array(V_0)

    for k in range(1, n_step + 1):
        Z_1 = np.random.normal(0, 1, n_front)
        Z_2 = np.random.normal(0, 1, n_front)

        dW2_corr = rho * np.sqrt(step_size) * Z_1 + np.sqrt(1 - rho ** 2) * np.sqrt(step_size)* Z_2

        V_front[:, k] = kappa * (theta - V_front[:, k-1]) * step_size + xi * np.sqrt(V_front[:, k-1]) * dW2_corr
        V_front[:, k] = np.maximum(V_front[:, k], 0)

        S_front[:, k] = S_front[:, k - 1] + drift * S_front[:, k - 1] * step_size + np.sqrt(V_front[:, k - 1]) * S_front[:, k - 1] * np.sqrt(step_size) * Z_1
